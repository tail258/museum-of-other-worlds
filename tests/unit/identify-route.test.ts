...
describe("POST /api/identify", () => {
  ...
  it("surfaces an invalid provider configuration without accepting the upload", async () => {
    process.env.AI_PROVIDER = "vendor-sdk";

    const response = await POST(createRequest(new File([new Uint8Array([137, 80, 78, 71])], "item.png", { type: "image/png" })));

    expect(response.status).toBe(500);
    await expect(response.json()).resolves.toMatchObject({ error: { code: "AI_CONFIGURATION_ERROR" } });
  });
});
